    ...
    const handleSearch = async (event) => {
        ...
        if (searchTerm) {
            try {
                ...
                setSearchResults(Array.isArray(response.data) ? response.data : []);  // Ensure response is an array
            } catch (error) {
                console.error("Error fetching search results", error);
                setSearchResults([]);  // Reset to an empty array if there's an error
            }
        }
    };
        ...
        <header className="max-w-screen-2xl mx-auto px-4 py-6">
            <nav className="flex justify-between items-center">
                ...
                <div className="flex items-center space-x-6 md:space-x-8 relative">
                    ...
                    {/* Sell button */}
                    <Link to="/sell-book" className="hidden sm:block bg-primary p-1 sm:px-6 px-2 items-center rounded-sm text-black hover:bg-opacity-80 transition">
                        Sell
                    </Link>
